...
async def get_comments(thread_id: str, user_login=Depends(require_auth), session: AsyncSession = Depends(get_session)):
    # Проверяем, существует ли thread (Entity или Topic)
    ...
    # Проверяем доступ пользователя (нужно реализовать логику проверки доступа к чату/каналу)
...
async def get_single_comment(
    ...
    # Определяем thread (Entity или Topic)
