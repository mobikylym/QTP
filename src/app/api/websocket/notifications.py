...
from src.app.storage.models import Entity, EntityTypeEnum, InfoEntity, InfoRequiredUser


async def send_entity_created(session: AsyncSession, room_type: str, room_id: str, entity_data: dict[str, Any]):
    ...
    message = {'event': 'entity_created', 'data': entity_data}
    await manager.send_to_room(room_type, room_id, message)


async def send_entity_updated(
    session: AsyncSession, room_type: str, room_id: str, updated_fields: dict[str, Any], entity_data: dict[str, Any]
):
...
async def send_topic_created(
    session: AsyncSession, topic_id: UUID, room_type: str, room_id: str, topic_data: dict[str, Any]
):
...
async def send_info_read(
    session: AsyncSession, entity_id: UUID, user_id: UUID, room_type: str, room_id: str, read_data: dict[str, Any]
):
